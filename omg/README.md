...
R package annotates cell types in plant scRNA-seq/snRNA-seq data by testing marker-gene
...
```r
library(omg)
omg("my_cluster_markers.csv") # Default
...
`pairwise/` folder — the same outputs as the command-line pipeline. 
...
**If your query species is new**: run OrthoFinder so your species' genes are placed into orthogroups.

```r
omg("my_markers.csv", orthogroups = "path/to/Orthogroups.tsv")
```

**To add a species into the reference panel**: also pass an updated markers table:
